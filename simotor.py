...
def _column_exists(c, table, column):
    """Check if a column already exists on a table (safe, no blind try/except)."""
...
def init_db():
    ...
    # Create motor WITHOUT foto/keterangan here on purpose: those are
    # added below via an idempotent migration step so that both fresh
    # installs and older databases (created before these columns existed)
    ...
    # This is the actual fix for the crash: previously this was wrapped in a
    # bare `try/except: pass`, which silently swallowed real errors (locked
    # db, permissions, etc). If the ALTER ever failed for a reason other than
    # "column already exists", the columns would simply never be added and
    # every later SELECT/UPDATE referencing them would crash with
    # "no such column: foto" / "no such column: keterangan".
    # Checking PRAGMA table_info first means we only ever run ALTER TABLE
    # when it's actually needed, and any unexpected failure surfaces loudly
    # instead of being hidden.
    if not _column_exists(c, "motor", "foto"):
        c.execute("ALTER TABLE motor ADD COLUMN foto BLOB")
    if not _column_exists(c, "motor", "keterangan"):
...
def seed_data(c):
    ...
    # bcrypt.hashpw returns bytes -- decode to str so it's stored consistently
    # as TEXT and round-trips the same way verify_login expects it to.
    c.execute(
        "INSERT INTO users (username,password,role,cabang) VALUES (?,?,?,?)",
        ('admin', bcrypt.hashpw(b'admin123', bcrypt.gensalt()).decode('utf-8'), 'admin', 'Pusat')
    )
    c.execute(
        "INSERT INTO users (username,password,role,cabang) VALUES (?,?,?,?)",
        ('petugas1', bcrypt.hashpw(b'petugas123', bcrypt.gensalt()).decode('utf-8'), 'petugas', 'Cabang Asoka')
    )
    ...
    c.executemany(
        "INSERT INTO motor (nopol,merek,jenis,tarif_jam,tarif_hari,status,cabang,foto,keterangan) "
        "VALUES (?,?,?,?,?,?,?,?,?)",
        motors
    )
    ...
    for t in trx_data:
        if t[7] == 'selesai':
            ...
            c.execute(
                "INSERT INTO pengembalian (transaksi_id,tgl_kembali,denda,total_bayar) VALUES (?,?,?,?)",
...
def verify_login(username, password):
    ...
    if user:
        ...
        try:
            ...
            # stored hash is malformed/corrupt -- treat as failed login rather
